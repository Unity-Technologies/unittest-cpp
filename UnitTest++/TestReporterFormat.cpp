...
void TestReporterFormat::ReportFailure(TestDetails const& details, char const* failure)
{
    ...
    OutputInternal(true, errorFormat, details.filename, details.lineNumber, details.testName, failure);
}
...
{
    if (failureCount > 0)
        OutputInternal(true, "FAILURE: %d out of %d tests failed (%d failures).\n", failedTestCount, totalTestCount, failureCount);
    else
        OutputInternal(false, "Success: %d tests passed.\n", totalTestCount);

    OutputInternal(false, "Test time: %.2f seconds.\n", secondsElapsed);
}

void TestReporterFormat::OutputInternal(bool failure, const char* log, ...)
{
	va_list vl;
	va_start(vl, log);
	Output(failure, log, vl);
	va_end(vl);
}
